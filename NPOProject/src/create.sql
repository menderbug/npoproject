...
CREATE TRIGGER donationDateCheck on donation
    ...
    if exists(select *
from inserted
where inserted.donation_date >= (select *
from currentdate)) 
...
CREATE TRIGGER pledgeDateCheck on pledge
    ...
    if exists(select *
from inserted
where inserted.pledge_date < (select *
from currentdate)) 
...
CREATE TRIGGER pledgesToDonations on currentDate
	...
	SELECT *
	FROM Pledge P
	WHERE P.pledge_date < (select date
	from currentdate)
...
CREATE TRIGGER recurringDonations on Donation
	...
	IF EXISTS (SELECT *
	FROM INSERTED
	WHERE recurrence = 'monthly' and dateadd(month, 1, donation_date)<=(select *
		from currentdate))
    throw 90000, 'You cant insert a monthly donation more than a month before the current date', 1;
	IF EXISTS (SELECT *
	FROM INSERTED
	WHERE recurrence = 'yearly' and dateadd(year, 1, donation_date)<=(select *
		from currentdate))
...
create trigger checkDateChange on currentdate 
	after update
	as
	if(abs(datediff(month, (select date
